    ...
    MenuItemNormalMode = MenuItem.extend({
        ...
        ok: function() {
            ...
            $.ajax({
                type: "post",
                // url: "word/getWord",
                url:"http://localhost:9523/eto/word/getWord",
                // data: ,
                dataType: "json", //返回数据类型
                success: function (data) {
                    $.each(data,function(i,obj){
                        // alert(obj.id);
                        arrayLetter.push(obj.word);
                        arrayLetter.push(obj.cn);
                    });
                },
                error:function(){
...
ig.module('game.entities.enemy').requires('impact.entity', 'impact.font', 'game.avenir-next', 'game.entities.explosion').defines(function() {
    EntityEnemy = ig.Entity.extend({
        ...
        getWordWithLength: function(l) {
            ...
            // }
            var str="";
            var rand=Math.floor(Math.random()*150)*2;
                w = ig.game.wordlist[l][rand];
            str="<tr class='removeWord'>"+
                "<td>"+ig.game.wordlist[l][rand]+"__</td>"+
                "<td>"+ig.game.wordlist[l][rand+1]+"</td>"+"</tr>";
            $("table").append(str);
            return w;
        },
        target: function() {
